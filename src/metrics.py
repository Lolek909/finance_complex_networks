...
def calculate_lead_lag_corr(series_a, series_b, max_lag=5):
    ...
    return float(max_c) if not np.isnan(max_c) else 0.0, best_lag
...
def test_volume_price_cross_corr(vol_a, price_b, max_lag=5):
    ...
    max_c = np.nanmax(corrs)
    return float(max_c) if max_c > 0 else 0.0, np.nanargmax(corrs) + 1


def evaluate_pair_unified(price_a, price_b, vol_a=None, max_lag=5):
    """
    Jeśli vol_a istnieje -> zwraca ocenę Advanced (Granger + MI + VolPrice).
    Jeśli vol_a to None -> zwraca ocenę Basic (Pearson).
    Zwraca zawsze krotkę o rozmiarze 5.
    """
    if vol_a is not None:
        ...
        if granger_score > 0:
            composite_weight = granger_score + mi_score + (vol_price_score * 0.5)
            lags = [lag for lag in [g_lag, mi_lag, vp_lag] if lag > 0]
            final_lag = max(set(lags), key=lags.count) if lags else g_lag
            return composite_weight, final_lag, granger_score, mi_score, vol_price_score
        return 0.0, 0, 0.0, 0.0, 0.0
    else:
        weight, lag = calculate_lead_lag_corr(price_a, price_b, max_lag)
        return weight, lag, 0.0, 0.0, 0.0